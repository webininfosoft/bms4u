<script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?key=AIzaSyCX1SGbjQbXFBLjBa-6BQtE6TWZ-Vi6oGI&sensor=false"></script>
    ...
    var customIcons = {
      Categories1: {
        icon: 'http://localhost/bms/img/icon1.png',
    ...
    function load() {
      ...
      // Change this depending on the name of your PHP file
       downloadUrl(strRoot+"/users/getxmldata", function(data) {
        var xml = data.responseXML;
        var markers = xml.documentElement.getElementsByTagName("marker");
        for (var i = 0; i < markers.length; i++) {
          var name = markers[i].getAttribute("name");
          var address = markers[i].getAttribute("address");
          var type = markers[i].getAttribute("phone");
          var point = new google.maps.LatLng(
              parseFloat(markers[i].getAttribute("lat")),
              parseFloat(markers[i].getAttribute("lng")));
          var html = "<b>" + name + "</b> <br/>" + address;
          var icon = customIcons[type] || {icon: 'http://localhost/bms/img/icon1.png',
        shadow: 'http://labs.google.com/ridefinder/images/mm_20_shadow.png'};
