...
function loadPermissions(userTypeId)
{
	...
	{	
		...
		location.href="<?php echo $this->html->url('/');?>Authentications/index/"+userTypeId;
	}
}
</script>
		...
		<div class="box-content">
			...
			<table width="100%" cellspacing="1" id="table_format" class='authtable' align="left" style="">
					<tr align="left">		    						
						...
						<th>Add</th>
						<th>Update</th>
						<th>Use App Offline</th>
						<th>Delete</th>						
					</tr>	
				<?php while(list($key,$val)=each($modules)) { $id=$val['company_module']['module_id'];?>
					<tr align="left">	
						...
						<td><input type="checkbox" name="perm[<?php echo $key;?>][perm_update]"  <?php echo $permissions[$id]['perm_update']?'checked':'' ?> class="check"></td>
					        <td><input type="checkbox" name="perm[<?php echo $key;?>][perm_app_offline]" <?php echo $permissions[$id]['perm_app_offline']?'checked':'' ?> class="check"></td>
						<td><input type="checkbox" name="perm[<?php echo $key;?>][perm_delete]"  <?php echo $permissions[$id]['perm_delete']?'checked':'' ?> class="check"></td>
